...
class RegisterForm(wtforms.Form):
    ...
    last_name = wtforms.fields.TextField('Last Name',
                    [wtforms.validators.Required()])
    date_born = wtforms.fields.TextField('date_born',
                    [wtforms.validators.Required()])
    email = wtforms.fields.TextField('Email',
                    [wtforms.validators.Required(),
                     wtforms.validators.Email()])
...
class User(Controller):
    ...
    def login(self):
        ...
        parser.container.password.description ='<a href="' + self.uri_for('user:reset_password') + '">Recuperar mi password</a>'
    ...
    def register(self):
        ...
        if self.request.method == "POST":
            ...
            date_born = parser.container.date_born.data.strip()
            ...
            if not user.activated:
                ...
                return self.redirect('/mayorista/register_ok')
    ...
    def social_login_complete(self, provider_name):
        ...
        continue_url = self.request.get('continue_url')
        if provider_name == "twitter":
            print 'dentro de Twitter'
            oauth_token = self.request.get('oauth_token')
            oauth_verifier = self.request.get('oauth_verifier')
            denied_log = self.request.get('denied')
            print denied_log
            if denied_log != '':
                print 'dentro de Twitter Denied'
            ...
            if self.user():
                ...
                if social_user:
                    ...
                    if continue_url:
                        ...
                        #return self.redirect('/')
                        return self.redirect('/mayorista/welcome')
                else:
                    uid = str(user_data['user_id'])
        ...
        # google, myopenid, yahoo OpenID Providers
            ...
            if current_user:
                ...
                print 'Federate Identity: '
                print current_user.federated_identity()
                print 'Federate Provider: '
                print current_user.federated_provider()
            else:
